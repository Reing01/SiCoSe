...
import type {
  requireResource as requireResourceType,
  requireRole as requireRoleType,
} from "../src/middleware/require-role.js";

let requireRole: typeof requireRoleType;
let requireResource: typeof requireResourceType;
...
before(async () => {
  ...
  ({ requireRole, requireResource } =
    await import("../src/middleware/require-role.js"));
});
...
describe("requireResource middleware", () => {
  it("returns 403 when user lacks access to the resource policy", () => {
    const request = {
      user: {
        id: "user-3",
        email: "secretaria@sicose.test",
    ...
    requireResource("cobranza")(request, response as never, () => {
      nextCalled = true;
    });
  ...
  it("calls next when user can access the resource policy", () => {
    const request = {
      user: {
        id: "user-4",
        email: "tesorero@sicose.test",
    ...
    requireResource("reportes")(request, response as never, () => {
      nextCalled = true;
    });
