...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    if (!session) {
      setState({ kind: 'error', message: 'Inicia sesion para registrar pagos.' })
      return
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(method === 'transferencia' ? 'Subiendo comprobante y registrando pago...' : 'Registrando pago...')
    setSuccess(null)
    ...
    try {
      ...
      setState((current) => {
        ...
        const nextDebts = current.debts.filter(
          (debt) => debt.id !== selectedDebt.id,
        )
        const nextDebt = nextDebts[0]
        ...
        return {
          ...current,
          debts: nextDebts,
          totalPending: Math.max(0, current.totalPending - selectedDebt.monto),
        }
      })
      setReceipt(null)
      setReference('')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'No fue posible registrar el pago.')
    } finally {
  ...
  return (
    ...
    >
      <header className="mx-auto flex max-w-7xl flex-col gap-4 px-4 pt-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <a href="/" className="inline-flex items-center gap-3 rounded-full border border-slate-200 bg-white/90 px-4 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0f3042] text-sm font-bold text-white">SC</div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#0f3042] dark:text-sky-300">SiCoSe</p>
            <p className="text-sm text-slate-500 dark:text-slate-300">Cobranza</p>
          </div>
        </a>
        <div className="flex flex-wrap items-center gap-3">
          <RoutePills variant={theme === 'dark' ? 'light' : 'dark'} />
          <ThemeToggle />
          <button type="button" onClick={handleLogout} className="inline-flex min-h-11 items-center rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
            Cerrar sesion
          </button>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6">
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-[#0f3042] dark:text-sky-300">Pagos</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-tight">Captura de pagos</h1>
        </div>

        {state.kind === 'loading' ? (
          <Card><CardContent className="p-6">Cargando adeudos pendientes...</CardContent></Card>
        ) : null}

        {state.kind === 'error' ? (
          <Card className="border-rose-200 bg-rose-50"><CardContent className="p-6 text-rose-800">{state.message}</CardContent></Card>
        ) : null}
          ...
          <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>{state.debts.length} adeudos cargados, {formatCurrency(state.totalPending)} por cobrar.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <Label htmlFor="debt">Adeudo</Label>
                <select id="debt" value={selectedDebtId} onChange={handleDebtChange} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                ...
                {selectedDebt ? (
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 dark:border-slate-800 dark:bg-slate-950/60">
                    <p className="font-semibold">{selectedDebt.ciudadano.nombre} {selectedDebt.ciudadano.apellido}</p>
                    <p>{selectedDebt.servicio.nombre} - {selectedDebt.periodo}</p>
                    <p className="font-semibold text-[#0f3042] dark:text-sky-300">{formatCurrency(selectedDebt.monto)}</p>
                  </div>
                ) : null}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>El folio se genera al confirmar el pago definitivo.</CardDescription>
              </CardHeader>
              <CardContent>
                <form className="space-y-5" onSubmit={handleSubmit}>
                  <div className="flex gap-2">
                    {(['efectivo', 'transferencia'] as const).map((item) => (
                      <Button key={item} type="button" variant={method === item ? 'default' : 'outline'} onClick={() => setMethod(item)}>
                  ...
                  {method === 'transferencia' ? (
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="receipt" type="file" accept="image/png,image/jpeg,application/pdf" onChange={handleReceiptChange} />
                        {receipt ? (
                  ...
                  {message ? (
                    <div role={success ? 'status' : 'alert'} className={cn('rounded-xl border p-4 text-sm', success ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800')}>
                      {message}
                    </div>
                  ) : null}
                  ...
                  {success ? (
                    <div className="rounded-xl border border-emerald-200 bg-white p-4 text-sm dark:bg-slate-950">
                      <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-700">Exito</p>
                      <p className="mt-2 text-2xl font-semibold">{success.folio ?? success.id}</p>
                    </div>
                  ) : null}

                  <Button type="submit" size="lg" className="w-full" disabled={isSubmitting || !selectedDebt}>
